...
hy = load_data("./data_www/hy_new_www_round2.pickle")

llm_vec = load_data("./data_www/llm_vec.pickle")
...
class vgae(nn.Module):
    ...
    def loss_vgae(self, edge_pos_pred, edge_index_batch, x_mean, x_std, reward=None):
        ...
        # print("kl_divergence:",kl_divergence.size(), loss_rec.mean(axis=1).size())
        # println()
...
def train(model: Model, x, edge_index, eps, model_1, optimizer_1,model_2, optimizer_2,lamb, alpha, beta, steps, node_ratio):
    ...
    x_2 = nn.Linear(1536, 96).cuda()(llm_vec.cuda())
    
    edge_pos_pred = model_1.forward_decoder(x_2,edge_index)
    ...
    #     # print("x_1:", x_1.size())
    #     # print("x:", x.size())
    #     file=open(r"./data_www/tmp_case_before.pickle","wb")
    ...
    #     # print("x_3:", x_3.size())
    #     file=open(r"./data_www/tmp_case_after.pickle","wb")
    ...
    # # print("##:", flow_trans.size(), flow_out.size())
    # # println()
    ...
    loss_3, link_loss_2 = model_1.loss_vgae(edge_pos_pred,edge_index, x_mean, x_std, reward=loss3.mean().item())
    # loss_2 =  loss_2
    ...
    return loss1.item(),loss_vage.item()
...
def test(model: Model, x, edge_index, model_1,y, final=False, task ="node"):   
    ...
    file=open(r"./data_www/iclr_vec1_wo_cl.pickle","wb")
...
if __name__ == '__main__':
    ...
    sample_size = 1370 # new york(1388)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        loss1,  loss3 = train(model, x, edge_index, eps, model_generative_1,optimizer_generative_1,model_generative_2,optimizer_generative_2, lamb, alpha, beta, 5, 0.2)
             
        now = t()                                     
        print(f'(T) | Epoch={epoch:03d}, loss1={loss1:.4f}'
    ...
    # printnl()
    print("=== Final ===")
    test(model, data.x, data.edge_index, model_generative_1,data.y, final=True)
